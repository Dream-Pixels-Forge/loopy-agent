...
# ---------------------------------------------------------------------------
# v0.7.10 - RealtimeSession (lazy websockets extra)
# ---------------------------------------------------------------------------
...
def _build_event(payload: dict[str, Any]) -> RealtimeEvent:
    """Translate a raw transport payload into a normalized ``RealtimeEvent``."""
    raw_type = payload.get("type", "unknown")
    try:
        etype = RealtimeEventType(raw_type)
    except ValueError:
        # Unknown event types become ``ERROR`` placeholders so consumers
        # can still observe them without crashing.
        etype = RealtimeEventType.ERROR
    return RealtimeEvent(type=etype, data=payload)
